...
if ($runExitCode -ne 0) {
	...
	if ($LASTEXITCODE -eq 0 -and $jobId) {
		...
		if ($LASTEXITCODE -eq 0 -and $annotation) {
			$failure += "`nReason: $annotation"
		}
	}

	$failure += "`n`nTag $version is pushed but no package was published. Once the cause is fixed:"
	$failure += "`n  gh run rerun $runId --repo $repoFullName --failed"
	Write-Error $failure
	exit 1
}
